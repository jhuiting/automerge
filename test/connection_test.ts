...
describe('Automerge.Connection', () => {
  ...
  function execution(links, steps) {
    const count = [], spies = [], conns = [], allConns = []

    for (const link of links) {
      const n1 = link[0], n2 = link[1]
    ...
    for (const conn of allConns) conn.open()
    ...
    for (const step of steps) {
      if (typeof step === 'function') {
        ...
        const msg = spies[step.from][step.to].getCall(count[step.from][step.to]).args[0]
  ...
  it('should concurrently exchange any missing documents', () => {
    const doc2 = Automerge.change(Automerge.init(), doc => doc.doc2 = 'doc2')
    nodes[1].setDoc('doc1', doc1)
  ...
  it('should tolerate duplicate message deliveries', () => {
    ...
    Automerge.merge(Automerge.init(), doc1)
    Automerge.merge(Automerge.init(), doc1)
    nodes[1].setDoc('doc1', doc1)
    nodes[2].setDoc('doc1', doc1)
